...
const updatedComplimentInput = document.querySelector('#updatedComplimentInput')
...
const addCompliment = evt => {
    ...
    axios.post("http://localhost:4000/api/compliment/", {newCompliment: complimentInput.value})
    .then(res => {
        let { data } = res
        showList(data)
...
const deleteCompliment = evt => {
    ...
    axios.delete(`http://localhost:4000/api/compliment/${evt.target.id}`)
    .then(response => {
        let { data } = response
        showList(data)
    })
